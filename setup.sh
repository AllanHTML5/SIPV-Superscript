...
get_public_ip() {
  # IP pública si hay salida a internet
  local ip=""
...
detect_allowed_hosts_delim() {
  # Detecta si settings.py usa split(",") o split() (espacios)
  local f=""
...
bootstrap_env_if_missing() {
  ...
  # ALLOWED_HOSTS: si es dev, preferimos permitir todo para evitar 400 por host-check
  if [[ "$mode" == "dev" ]]; then
    hosts="*"
  else
    # prod: lista con IPs detectadas
    if [[ -n "$public_ip" ]]; then
      if [[ "$delim" == "space" ]]; then
  ...
  # Si DB_USER no existe, dejamos default sipvuser del example
...
ensure_env_has_generated_values() {
  # Si ya existe .env, solo rellenamos faltantes
  local private_ip public_ip
  private_ip="$(get_private_ip)"
  public_ip="$(get_public_ip || true)"
...
install_docker_if_missing() {
  ...
  run_as_root "apt-get update -y"
  run_as_root "apt-get remove -y docker.io docker-doc docker-compose docker-compose-v2 podman-docker containerd runc || true"
...
create_superuser_always() {
  ...
  # Asegurar settings para cualquier comando manage.py
...
write_summary() {
  ...
  # Si WEB_BIND_IP es 0.0.0.0, lo normal es acceder por la IP pública
  if [[ "${WEB_BIND_IP:-0.0.0.0}" == "0.0.0.0" ]]; then
...
apply_migrations() {
  ...
  # Esto aplica migraciones de forma explícita y falla si hay error real
  $cmd run --rm \
    -e DJANGO_SETTINGS_MODULE="backend.settings" \
...
start_stack() {
  ...
  # migraciones antes de crear superuser
  apply_migrations
  collectstatic_if_prod
