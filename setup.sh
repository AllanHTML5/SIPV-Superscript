...
services:
  mysql_primary:
    ...
    healthcheck:
      test: ["CMD", "mysqladmin", "ping", "-uroot", "--password=${MYSQL_ROOT_PASSWORD}"]
      interval: 5s
      timeout: 3s
      retries: 20
    networks: [sipv_net]
...
mysql_root_probe_nopass() {
  docker exec -i mysql_primary mysql -uroot -e "SELECT 1;" >/dev/null 2>&1
}

mysql_app_probe() {
  docker exec -i mysql_primary mysql -u"${DB_USER}" -p"${DB_PASSWORD}" -D "${DB_NAME}" -e "SELECT 1;" >/dev/null 2>&1
}

mysql_set_root_password() {
  docker exec -i mysql_primary mysql -uroot <<SQL
...
mysql_autofix_root_1045() {
  ...
  warn "MySQL root auth falló (ERROR 1045). Reinicializando MySQL (down -v)..."

  $cmd down -v --remove-orphans || true
  docker rm -f mysql_primary mysql_replica >/dev/null 2>&1 || true
  $cmd up -d --force-recreate mysql_primary mysql_replica
...
ensure_mysql_db_user() {
  ...
  if echo "$out" | grep -q "ERROR 1045"; then
    if mysql_root_probe_nopass; then
      mysql_set_root_password
      out="$(mysql_root_probe_output)"
    else
      mysql_autofix_root_1045
      out="$(mysql_root_probe_output)"
      if echo "$out" | grep -q "ERROR 1045" && mysql_root_probe_nopass; then
        mysql_set_root_password
        out="$(mysql_root_probe_output)"
      fi
    fi
...
SQL
  ...
  if mysql_app_probe; then
    warn "No se pudo autenticar como root, pero la conexión con DB_USER funciona. Continuando."
    return 0
  fi
  ...
  die "Fallo al conectar a MySQL. Revisa .env (valores/CRLF) y el estado del contenedor."
}
...
main() {
  ...
  normalize_lf_file "$ENV_EXAMPLE_FILE"
  normalize_lf_file "$ENV_FILE"
